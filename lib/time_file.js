...
function TimeFile(config) {
  ...
  this.symlink = (config.get("symlink")) ? path_module.join(this.cwd, config.get("symlink")) : "";
  this.symlink = "";
  if (config.get("symlink")) {
    this.symlink = config.get("symlink");
    if (!this.symlink.match(ROOT_PATH_RE)) {
...
function archive(path) {
  ...
  inp.on("end", function () {
    // TBD emit something here for other listeners that might want to act on archived file? (e.g. archival/S3 transfer)
    fs.unlink(path);
  });
